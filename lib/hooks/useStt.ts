...
interface SttFrame {
  type: "partial" | "final";
  text: string;
  /** Optional EN translation when the server fans out STT→Translate. */
  translation?: string;
}
...
export function useStt(opts: UseSttOptions = {}): UseSttResult {
  ...
  const connectWs = React.useCallback(
    async (sendFn: (chunk: ArrayBuffer) => void): Promise<WebSocket> => {
      ...
      ws.onmessage = (ev) => {
        try {
          const frame = JSON.parse(typeof ev.data === "string" ? ev.data : "") as SttFrame;
          if (frame.type === "partial") {
            addPartial(frame.text);
          } else if (frame.type === "final") {
            promotePartial(frame.text, frame.translation);
          }
        } catch {
          // Non-JSON frames are ignored — the server sends only JSON envelopes.
        }
      };
      ws.onerror = () => {
    ...
    [addPartial, promotePartial, setStatus, url],
  );
  ...
  const start = React.useCallback(async (): Promise<void> => {
    ...
    i.teardown = (): void => {
      ...
      i.stream = null;
      if (i.ws) {
        try { i.ws.close(); } catch { /* noop */ }
      }
      i.ws = null;
    };
  }, [connectWs, deviceId, mutedDb, mutedWindowMs, setStatus]);
